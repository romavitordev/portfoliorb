      ...
      <section className="relative overflow-hidden pt-28 md:pt-36">
        ...
        <div className="container-page relative">
          ...
          <div className="mt-8 flex flex-wrap gap-2">
            <span className="chip border-clay/50 text-clay">
              {rotulosCategoria[projeto.categoria]}
            </span>
            <span className="chip border-creme/20 text-creme/55">
              {rotulosNatureza[projeto.natureza]}
            </span>
            <span className="chip border-creme/20 text-creme/55">{projeto.ano}</span>
          </div>

          <h1 className="t-h1 mt-6 font-display">{projeto.nome}</h1>
          <p className="mt-4 font-mono text-[0.7rem] uppercase tracking-[0.2em] text-creme/45">
            {projeto.cliente}
          </p>
          <p className="t-lead measure mt-8 text-creme/70">{projeto.intro}</p>
        ...
        <div className="container-wide">
          <Reveal>
            <div className="relative aspect-[16/9] overflow-hidden rounded-2xl border border-linha">
              ...
              <div
                aria-hidden
                className="absolute inset-0 bg-gradient-to-t from-carvao/70 via-transparent to-transparent"
              />
            </div>
          </Reveal>
        </div>
            ...
            <ul className="mt-8 grid gap-5 md:grid-cols-3">
              {projeto.resultados.map((r, i) => (
                <Reveal key={r} delay={i * 0.06}>
                  <li className="flex gap-4 border-l-2 border-clay/50 pl-5">
                    <p className="text-sm leading-relaxed text-creme/70">{r}</p>
                  </li>
                </Reveal>
              ))}
            </ul>
        ...
        <div className="grid gap-px overflow-hidden rounded-2xl border border-linha bg-linha sm:grid-cols-2">
          {anterior && (
            <Link
              ...
              className="group bg-grafite p-8 transition-colors duration-500 hover:bg-grafite/55"
            >
              <span className="inline-flex items-center gap-2 font-mono text-[0.64rem] uppercase tracking-[0.2em] text-creme/40">
                <ArrowLeft size={13} /> Anterior
              </span>
              <p className="mt-3 font-display text-xl tracking-tight transition-colors group-hover:text-clay">
                {anterior.nome}
              </p>
            </Link>
          )}
          ...
          {proximo && (
            <Link
              ...
              className="group bg-grafite p-8 text-right transition-colors duration-500 hover:bg-grafite/55"
            >
              <span className="inline-flex items-center gap-2 font-mono text-[0.64rem] uppercase tracking-[0.2em] text-creme/40">
