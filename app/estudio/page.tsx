    ...
    <>
        ...
        <div className="container-page relative">
          <Reveal>
            ...
            <h1 className="t-h1 mt-6 font-display">
              Duas pessoas, <span className="texto-espectro">{projetos.length} projetos</span> e nenhuma
              camada de gerência
            </h1>
        ...
        <div className="container-wide">
          <Reveal>
            <div className="relative aspect-[21/9] overflow-hidden rounded-2xl border border-linha">
              ...
              <div
                aria-hidden
                className="absolute inset-0 bg-gradient-to-t from-abismo via-abismo/25 to-transparent"
              />
            </div>
          </Reveal>
        </div>
            ...
            <Reveal key={socio.id} delay={i * 0.06}>
              <article className="surface grid gap-10 p-8 md:p-12 lg:grid-cols-[1fr_1fr]">
                <div>
                  ...
                  <p className="mt-3 font-mono text-[0.7rem] uppercase tracking-[0.22em] text-ciano">
                    {socio.papel}
                  </p>
                  <p className="mt-7 text-lg leading-relaxed text-luz/80">{socio.resumo}</p>
                  ...
                  <ul className="mt-5 space-y-3.5">
                    {socio.faz.map((item) => (
                      <li key={item} className="flex items-start gap-3 text-sm text-luz/70">
                        <span
                          aria-hidden
                          className="mt-2 h-1 w-1 shrink-0 rounded-full bg-violeta"
                        />
                        {item}
                      </li>
                    ))}
      ...
      <section className="relative section-y-tight bg-nevoa/30">
        ...
        <div className="container-page">
            ...
            {stack.map((grupo, g) => (
              <Reveal key={grupo.grupo} delay={g * 0.04}>
                <div className="grid gap-6 md:grid-cols-[13rem_1fr]">
                  <h3 className="font-mono text-[0.68rem] uppercase tracking-[0.24em] text-ciano md:pt-1">
                    {grupo.grupo}
                  </h3>
                  ...
                  <dl className="grid gap-x-8 gap-y-5 sm:grid-cols-2 lg:grid-cols-3">
                    {grupo.itens.map((item) => (
                      <div key={item.nome} className="border-l border-linha pl-5">
                        ...
                        <dd className="mt-1 text-sm text-luz/55">{item.nota}</dd>
                      </div>
                    ))}
                  </dl>
                </div>
          ...
          <Reveal>
            <p className="measure mt-14 border-l-2 border-violeta/50 pl-5 text-sm leading-relaxed text-luz/60">
              {stackNota}
            </p>
            <p className="mt-8 text-sm text-luz/45">
              Todo o código do estúdio é público em{' '}
              <a
                href={brand.github}
                target="_blank"
                rel="noreferrer noopener"
                className="text-ciano underline underline-offset-4"
              >
