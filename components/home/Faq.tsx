          ...
          {faq.map((item, i) => (
            <Reveal key={item.q} delay={i * 0.03}>
              <details className="group border-b border-linha">
                <summary className="flex cursor-pointer list-none items-start justify-between gap-6 py-6 text-left transition-colors hover:text-ciano [&::-webkit-details-marker]:hidden">
                  <h3 className="text-base font-medium md:text-lg">{item.q}</h3>
                  <span
                    aria-hidden
                    className="relative mt-2 h-3 w-3 shrink-0 text-ciano"
                  >
                ...
                <p className="measure pb-7 text-sm leading-relaxed text-luz/60">{item.a}</p>
              </details>
            </Reveal>
          ))}
        </div>
